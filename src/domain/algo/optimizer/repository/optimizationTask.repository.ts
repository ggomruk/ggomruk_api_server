...
import {
  OptimizationTask,
  OptimizationTaskDocument,
} from '../schema/optimizationTask.schema';

@Injectable()
export class OptimizationTaskRepository {
  constructor(
    @InjectModel(OptimizationTask.name)
    private optimizationTaskModel: Model<OptimizationTaskDocument>,
  ) {}

  async create(
    optimizationTask: Partial<OptimizationTask>,
  ...
  async findByUserId(userId: string): Promise<OptimizationTask[]> {
    return this.optimizationTaskModel
      .find({ userId })
      .sort({ createdAt: -1 })
      .exec();
  }
  ...
  async updateStatus(
    ...
    return this.optimizationTaskModel
      .findOneAndUpdate({ optimizationId }, update, { new: true })
      .exec();
  }
}
