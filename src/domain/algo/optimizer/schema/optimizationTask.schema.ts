...
@Schema({ _id: false })
class OptimizationStrategyParamSchema
  extends Document
  implements IOptimizationStrategyParam
{
  @Prop({ required: true })
  name: string;
  @Prop({ required: true })
...
@Schema({ _id: false })
class OptimizationStrategySchema
  extends Document
  implements IOptimizationStrategy
{
  @Prop({ required: true })
  id: string;
  @Prop({ required: true })
  type: string;
  @Prop({
    type: [SchemaFactory.createForClass(OptimizationStrategyParamSchema)],
  })
...
export class OptimizationTask {
  ...
  @Prop({
    type: SchemaFactory.createForClass(OptimizationParamsSchema),
    required: true,
  })
...
export const OptimizationTaskSchema =
  SchemaFactory.createForClass(OptimizationTask);
