...
export class UserDTO {
  @ApiProperty({
    description:
      'Username (min 8 characters, at least one lowercase letter and one number)',
    example: 'john_doe123',
    minLength: 8,
  })
  @IsNotEmpty()
  ...
  @Matches(/^(?=.*[a-z])(?=.*\d)[A-Za-z\d@$!%*?&_-]{8,}$/, {
    message:
      'Username must be at least 8 characters with at least one lowercase letter and one number',
  })
  username: string;
