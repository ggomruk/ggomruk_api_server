  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  handleBacktestUnsubscribe(
    ...
    this.logger.log(
      `Client ${client.id} unsubscribed from backtest updates for user ${userId}`,
    );
  ...
  private setupRedisSubscriptions() {
    ...
    this.backtestPubSub.onProgress((data) => {
      this.logger.debug(
        `Broadcasting progress for backtest ${data.backtestId}: ${data.progress}%`,
      );
    ...
    this.backtestPubSub.onOptimizationProgress((data) => {
      this.logger.debug(
        `Broadcasting progress for optimization ${data.optimizationId}: ${data.progress}%`,
      );
    ...
    this.backtestPubSub.onOptimizationComplete((data) => {
      this.logger.log(
        `Broadcasting completion for optimization ${data.optimizationId}`,
      );
    ...
    this.backtestPubSub.onWalkforwardProgress((data) => {
      this.logger.debug(
        `Broadcasting progress for walkforward ${data.walkforwardId}: ${data.progress}%`,
      );
    ...
    this.backtestPubSub.onWalkforwardComplete((data) => {
      this.logger.log(
        `Broadcasting completion for walkforward ${data.walkforwardId}`,
      );
  ...
  private broadcastToUser(userId: string, event: string, data: any) {
    ...
    if (!this.server) {
      this.logger.warn(
        `WebSocket server not initialized, cannot broadcast ${event} to user ${userId}`,
      );
      return;
    }
    ...
    this.logger.log(
      `📤 Broadcasting ${event} to ${socketIds.length} socket(s) for user ${userId}: [${socketIds.join(', ')}]`,
    );
